...
export const FileList = ({ files, isLoading, onDownload, onDelete }: FileListProps) => {
  if (isLoading) {
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {[...Array(8)].map((_, i) => (
          <div key={i} className="glass-card h-40 rounded-2xl animate-pulse bg-card text-card-foreground" />
        ))}
      </div>
    );
  }
  ...
  if (files.length === 0) {
    return (
      ...
      >
        <div className="p-6 bg-card rounded-full mb-4 shadow-sm border border-border">
